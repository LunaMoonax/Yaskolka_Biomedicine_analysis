...
# Stipriausių efekto dydžių apžvalginė analizė
...
ggplot(cohensd_df, aes(x=relation_to_island)) +
  labs(title = "CpG pasiskirstymas tarp regionų (atrinktos pagal stiprų Cohen's d efekto dydį)", x = "Regionas", y = "CpG pozicijų kiekis regione") +
  theme(plot.title = element_text(hjust=0.5, size=12)) +
  geom_bar(fill = "slateblue")

ggplot(r_df, aes(x=relation_to_island)) +
  labs(title = "CpG pasiskirstymas tarp regionų (atrinktos pagal stiprų r (biserial rank) efekto dydį)", x = "Regionas", y = "CpG pozicijų kiekis regione") +
  theme(plot.title = element_text(hjust=0.5, size=12)) +
  geom_bar(fill = "slateblue")
...
eff_val_r_strong <- eff_val_r_strong[order(abs(eff_val_r_strong$eff_val_r_strong), decreasing = TRUE),]
